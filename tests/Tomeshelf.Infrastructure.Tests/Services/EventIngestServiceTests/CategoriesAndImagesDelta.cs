...
public class EventIngestServiceCategoriesAndImagesDeltaTests
{
    ...
    public async Task UpsertAsync_ReplacesImages_AndSynchronizesCategories()
    {
        ...
        // Assert
        var person = await db.People.Include(p => p.Images)
                             .Include(p => p.Categories)
                             .ThenInclude(pc => pc.Category)
                             .SingleAsync(p => p.ExternalId == "P1", TestContext.Current.CancellationToken);
        person.Images.Should()
              .HaveCount(1);
        person.Images.Single()
              .Big.Should()
              .Be("b2");
        var catIds = person.Categories.Select(c => c.Category.ExternalId)
                           .OrderBy(x => x)
                           .ToArray();
        catIds.Should()
              .BeEquivalentTo("B", "C");
    ...
    public async Task UpsertAsync_AddsAndUpdatesSchedules_AndVenueLocations()
    {
        ...
        ea.Schedules.Single(s => s.ExternalId == "S1")
          ...
          .Be("Talk Updated");
        ea.Schedules.Single(s => s.ExternalId == "S1")
          .VenueLocation.Name.Should()
          .Be("Hall Updated");
        ea.Schedules.Single(s => s.ExternalId == "S2")
          .VenueLocation.Should()
          .BeNull();
    }
}