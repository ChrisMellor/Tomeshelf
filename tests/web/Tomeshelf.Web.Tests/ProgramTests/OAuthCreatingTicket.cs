...
public class OAuthCreatingTicket
{
    ...
    public async Task SetsSessionValuesFromLoginHint()
    {
        ...
        // Assert
        httpContext.Session.GetString(ProgramTestHarness.ClientIdKey).ShouldBe("client-id");
        httpContext.Session.GetString(ProgramTestHarness.ClientSecretKey).ShouldBe("client-secret");
        httpContext.Session.GetString(ProgramTestHarness.RefreshTokenKey).ShouldBe("refresh");
        httpContext.Session.GetString(ProgramTestHarness.UserEmailKey).ShouldBe("hint@example.test");
    }
    ...
    public async Task UsesConfiguredEmailWhenNoLoginHint()
    {
        ...
        // Assert
        httpContext.Session.GetString(ProgramTestHarness.UserEmailKey).ShouldBe("config@example.test");
    }
    ...
    public async Task WhenRefreshTokenMissing_SetsError()
    {
        ...
        // Assert
        httpContext.Session.GetString(ProgramTestHarness.ErrorKey).ShouldBe("Google did not return a refresh token. Re-run the flow and accept offline access.");
