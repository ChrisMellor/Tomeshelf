...
public class HomeControllerTests
{
    ...
    public async Task Index_ReturnsSortedEndpointsAndApis()
    {
        ...
        var apis = new List<ApiServiceDescriptor>
        {
            new("mcm", "MCM", "https://mcm.test")
        };
        ...
        var model = view.Model.Should().BeOfType<ExecutorConfigurationViewModel>().Subject;
        model.Endpoints.Should().HaveCount(2);
        model.Endpoints[0].Name.Should().Be("Alpha");
        model.ApiServices.Should().ContainSingle();
        model.ApiServices[0].ServiceName.Should().Be("mcm");
        model.Endpoints[1].HeadersDisplay.Should().Be("X-Test:value");
    }
    ...
    public async Task Create_Get_ReturnsEditorDefaults()
    {
        ...
        var view = result.Should().BeOfType<ViewResult>().Subject;
        var model = view.Model.Should().BeOfType<ExecutorConfigurationViewModel>().Subject;
        model.Editor.Enabled.Should().BeTrue();
        model.Editor.Method.Should().Be("POST");
    }
    ...
    public async Task Ping_Get_ReturnsPingDefaults()
    {
        ...
        var view = result.Should().BeOfType<ViewResult>().Subject;
        var model = view.Model.Should().BeOfType<ExecutorConfigurationViewModel>().Subject;
        model.Ping.Method.Should().Be("GET");
    }
    ...
    public async Task Toggle_Post_UpdatesOptionsAndRefreshes()
    {
        ...
        A.CallTo(() => store.SaveAsync(A<ExecutorOptions>._, A<CancellationToken>._)).Returns(Task.CompletedTask);
        A.CallTo(() => scheduler.RefreshAsync(A<ExecutorOptions>._, A<CancellationToken>._)).Returns(Task.CompletedTask);

        var result = await controller.Toggle(true, CancellationToken.None);

        options.Enabled.Should().BeTrue();
        controller.TempData["StatusMessage"].Should().Be("Scheduler enabled.");
    ...
    public async Task Create_Post_InvalidModel_ReturnsView()
    {
        ...
        result.Should().BeOfType<ViewResult>().Which.ViewName.Should().Be("Create");
        A.CallTo(() => store.SaveAsync(A<ExecutorOptions>._, A<CancellationToken>._)).MustNotHaveHappened();
    }
    ...
    public async Task Create_Post_DuplicateName_ReturnsViewWithError()
    {
        ...
        result.Should().BeOfType<ViewResult>().Which.ViewName.Should().Be("Create");
        controller.ModelState[nameof(EndpointEditorModel.Name)]!.Errors.Should().NotBeEmpty();
        A.CallTo(() => store.SaveAsync(A<ExecutorOptions>._, A<CancellationToken>._)).MustNotHaveHappened();
    }
    ...
    public async Task Create_Post_Valid_AddsEndpointAndRedirects()
    {
        ...
        result.Should().BeOfType<RedirectToActionResult>();
        controller.TempData["StatusMessage"].Should().Be("Added endpoint 'Ping'.");
        saved.Should().NotBeNull();
        saved!.Endpoints.Should().ContainSingle();
        saved.Endpoints[0].Url.Should().Be("http://example.test/");
        saved.Endpoints[0].Headers.Should().ContainKey("X-Test");
    }
    ...
    public async Task Edit_Post_Valid_UpdatesEndpoint()
    {
        ...
        result.Should().BeOfType<RedirectToActionResult>();
        controller.TempData["StatusMessage"].Should().Be("Updated endpoint 'Updated'.");
        options.Endpoints[0].Name.Should().Be("Updated");
        options.Endpoints[0].Url.Should().Be("http://updated.test/");
    }
    ...
    public async Task Delete_Post_RemovesEndpoint()
    {
        ...
        result.Should().BeOfType<RedirectToActionResult>();
        controller.TempData["StatusMessage"].Should().Be("Deleted endpoint 'DeleteMe'.");
    ...
    public async Task GetDiscoveredEndpoints_ReturnsMappedPayload()
    {
        ...
        var endpoints = new List<ExecutorDiscoveredEndpoint>
        {
            new("id", "POST", "/path", "Display", "Desc", true, "Group")
        };
        ...
        var item = payload.Should().ContainSingle().Subject;
        item.GetType().GetProperty("id")!.GetValue(item).Should().Be("id");
        item.GetType().GetProperty("groupName")!.GetValue(item).Should().Be("Group");
    }
    ...
    public async Task Ping_Post_InvalidUrl_ReturnsViewWithErrors()
    {
        ...
        result.Should().BeOfType<ViewResult>().Which.ViewName.Should().Be("Ping");
        A.CallTo(() => pingService.SendAsync(A<Uri>._, A<string>._, A<Dictionary<string, string>?>._, A<CancellationToken>._))
            .MustNotHaveHappened();
        controller.ModelState[nameof(EndpointPingModel.Url)]!.Errors.Should().NotBeEmpty();
    }
    ...
    public async Task Ping_Post_Success_ReturnsResultAndStatusMessage()
    {
        ...
        var view = result.Should().BeOfType<ViewResult>().Subject;
        var viewModel = view.Model.Should().BeOfType<ExecutorConfigurationViewModel>().Subject;
        viewModel.PingResult.Should().NotBeNull();
        viewModel.PingResult!.ResponseBody.Should().EndWith("... (truncated)");
        controller.TempData["StatusMessage"].Should().Be("Ping succeeded with status 200 (5 ms).");
    }
        ...
        out IEndpointPingService pingService)
    {
        ...
        A.CallTo(() => storeFake.GetAsync(A<CancellationToken>._)).Returns(options);
        A.CallTo(() => discoveryFake.GetApisAsync(A<CancellationToken>._)).Returns(apis);
        ...
        A.CallTo(() => tempDataProvider.LoadTempData(httpContext)).Returns(new Dictionary<string, object?>());
