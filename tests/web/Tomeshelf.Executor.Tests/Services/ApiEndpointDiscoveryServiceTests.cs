...
public class ApiEndpointDiscoveryServiceTests
{
    ...
    public async Task GetApisAsync_ReturnsDistinctOrderedServices()
    {
        ...
        results[0].ServiceName.Should().Be("alpha");
        results[0].BaseAddress.Should().Be("https://alpha.test");
        results[0].DisplayName.Should().Be("alpha (HTTP)");
        results[1].ServiceName.Should().Be("beta");
        results[1].BaseAddress.Should().Be("https://beta.test");
        results[2].ServiceName.Should().Be("gamma");
    }
    ...
    public async Task GetEndpointsAsync_WhenResponseOk_ReturnsEndpoints()
    {
        ...
        {
            new("id-1", "POST", "/path", "Display", "Desc", true, "Group")
        };
        ...
        handler.Requests.Single().RequestUri.Should().Be(new Uri("https://api.test/.well-known/executor-endpoints"));
    }
