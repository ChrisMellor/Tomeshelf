...
public class GetApisAsync
{
    ...
    public async Task ReturnsDistinctOrderedServices()
    {
        ...
        results[0].ServiceName.ShouldBe("alpha");
        results[0].BaseAddress.ShouldBe("https://alpha.test");
        results[0].DisplayName.ShouldBe("alpha (HTTP)");
        results[1].ServiceName.ShouldBe("beta");
        results[1].BaseAddress.ShouldBe("https://beta.test");
        results[2].ServiceName.ShouldBe("gamma");
    }
