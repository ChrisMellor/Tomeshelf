...
public class AddApplicationServices
{
    ...
    public void RegistersExpectedServices()
    {
        ...
        provider.GetRequiredService<IGuestMapper>().ShouldBeOfType<GuestMapper>();
        provider.GetRequiredService<IGuestsService>().ShouldBeOfType<GuestsService>();
        provider.GetRequiredService<IEventService>().ShouldBeOfType<EventService>();
