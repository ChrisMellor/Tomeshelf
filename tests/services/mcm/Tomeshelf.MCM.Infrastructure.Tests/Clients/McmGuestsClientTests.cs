...
""";
        ...
        factory.LastName.Should().Be(McmGuestsClient.HttpClientName);
        handler.RequestMessage.Should().NotBeNull();
        handler.RequestMessage!.Method.Should().Be(HttpMethod.Post);
        handler.RequestMessage.RequestUri.Should().Be(new Uri("https://example.test/api/people?key=mcm%20spring"));

        handler.RequestBody.Should().Be("{}");
        handler.RequestContentType.Should().Be("application/json");

        guests.Should().HaveCount(2);
        guests[0].Name.Should().Be("Ada Lovelace");
        guests[0].Description.Should().Be("Bio");
        guests[0].ProfileUrl.Should().Be("https://imdb.example/ada");
        guests[0].ImageUrl.Should().Be("https://images.example/ada.png");

        guests[1].Name.Should().Be("Solo");
        guests[1].Description.Should().Be("Known for");
        guests[1].ProfileUrl.Should().BeEmpty();
        guests[1].ImageUrl.Should().BeEmpty();
    }
