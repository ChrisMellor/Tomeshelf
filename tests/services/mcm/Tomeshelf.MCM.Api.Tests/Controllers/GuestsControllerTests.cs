...
public class GuestsControllerTests
{
    ...
    public async Task Get_ReturnsBadRequest_WhenPageIsLessThanOne()
    {
        ...
        var result = await controller.Get("event-1", CancellationToken.None, page: 0, pageSize: 50);
        ...
        problem.Detail.Should().Be("page must be >= 1");
    }
    ...
    public async Task Get_ReturnsBadRequest_WhenPageSizeIsOutOfRange()
    {
        ...
        var result = await controller.Get("event-1", CancellationToken.None, page: 1, pageSize: 401);

        var badRequest = result.Should().BeOfType<ObjectResult>().Subject;
        var problem = badRequest.Value.Should().BeOfType<ValidationProblemDetails>().Subject;
        problem.Detail.Should().Be("pageSize must be between 1 and 400");
    }
    ...
    public async Task Get_ReturnsOk_WithPagedResults()
    {
        ...
        var result = await controller.Get("event-1", CancellationToken.None, page: 2, pageSize: 25, eventName: "Event", includeDeleted: true);

        var ok = result.Should().BeOfType<OkObjectResult>().Subject;
        ok.Value.Should().BeSameAs(expected);

        A.CallTo(() => queryHandler.Handle(A<GetGuestsQuery>.That.Matches(query =>
            query.EventId == "event-1" &&
            query.Page == 2 &&
            query.PageSize == 25 &&
            query.EventName == "Event" &&
            query.IncludeDeleted), A<CancellationToken>._))
    ...
    public async Task Sync_ReturnsNotFound_WhenSyncResultIsNull()
    {
        ...
        notFound.Value.Should().Be("event-1");
