...
public class AddInfrastructureServices
{
    ...
    public async Task RegistersExpectedServices()
    {
        ...
        {
            ["ConnectionStrings:shiftdb"] = @"Server=(localdb)\mssqllocaldb;Database=ShiftTest;Trusted_Connection=True;"
        });
        ...
        // Assert
        provider.GetRequiredService<TomeshelfShiftDbContext>().ShouldNotBeNull();
        provider.GetRequiredService<IShiftSettingsRepository>().ShouldBeOfType<ShiftSettingsRepository>();
        provider.GetRequiredService<IShiftWebSession>().ShouldBeOfType<ShiftWebSession>();
        provider.GetRequiredService<IShiftWebSessionFactory>().ShouldBeOfType<ShiftWebSessionFactory>();
        provider.GetRequiredService<IGearboxClient>().ShouldBeOfType<GearboxClient>();
        provider.GetRequiredService<IShiftKeySource>().ShouldBeOfType<XShiftKeySource>();
        provider.GetRequiredService<ISecretProtector>().ShouldBeOfType<DataProtectionSecretProtector>();
        provider.GetRequiredService<IClock>().ShouldBeOfType<SystemClock>();
        ...
        xClient.DefaultRequestHeaders.Accept.ToString().ShouldContain("application/json");

        var authClient = factory.CreateClient(XAppOnlyTokenProvider.HttpClientName);
        authClient.DefaultRequestHeaders.Accept.ToString().ShouldContain("application/json");
    }
