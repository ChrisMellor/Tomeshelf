...
public class BuildRedeemBodyAsync
{
    ...
    public async Task ReturnsMatchingOptions()
    {
        ...
        options[0].Service.ShouldBe("psn");
        options[0].Title.ShouldBe("Golden Keys");
        options[0].DisplayName.ShouldBe("PlayStation");
        options[0].FormBody.ShouldContain("archway_code_redemption%5Bservice%5D=psn");
        options[0].FormBody.ShouldContain("archway_code_redemption%5Btitle%5D=Golden+Keys");
        options[0].FormBody.ShouldContain("archway_code_redemption%5Bcode%5D=CODE-123");
        options[0].FormBody.ShouldContain("utf8=%E2%9C%93");
    }
    ...
    public async Task ReturnsNullDisplayName_WhenMissingButton()
    {
        ...
        // Assert
        options.ShouldHaveSingleItem();
        options[0].DisplayName.ShouldBeNull();
    }
}
