...
public class RedeemCodeAsync
{
    ...
    public async Task ReturnsAccountMisconfigured_WhenUserMissingFields()
    {
        ...
        var users = new List<(int Id, string Email, string Password, string Service)>
        {
            (1, string.Empty, "password", "psn")
        };
        ...
        results[0].ErrorCode.ShouldBe(RedeemErrorCode.AccountMisconfigured);
        results[0].Message.ShouldBe("Missing email, password, or service.");
    ...
    public async Task RedeemsAllOptions_AndReturnsSuccess()
    {
        ...
        var users = new List<(int Id, string Email, string Password, string Service)>
        {
            (7, "user@example.com", "password", "psn")
        };
        ...
        {
            new RedemptionOption("psn", "Title", null, "body-1"),
            new RedemptionOption("psn", "Title", null, "body-2")
        };
    ...
    public async Task MapsKnownErrors(Exception exception, RedeemErrorCode expectedCode, string expectedMessage)
    {
        ...
        var users = new List<(int Id, string Email, string Password, string Service)>
        {
            (8, "user@example.com", "password", "psn")
        };
        ...
        results.ShouldHaveSingleItem();
        results[0].Success.ShouldBeFalse();
        results[0].ErrorCode.ShouldBe(expectedCode);
        results[0].Message.ShouldBe(expectedMessage);
    }
