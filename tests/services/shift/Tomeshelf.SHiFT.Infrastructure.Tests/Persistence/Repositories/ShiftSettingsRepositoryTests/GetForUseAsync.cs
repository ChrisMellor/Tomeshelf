...
public class GetForUseAsync
{
    ...
    public async Task HandlesCryptographicFailure()
    {
        ...
        context.ShiftSettings.AddRange(
            new SettingsEntity { Id = 1, Email = "user1@example.com", DefaultService = "psn", EncryptedPassword = "enc-ok" },
            new SettingsEntity { Id = 2, Email = "user2@example.com", DefaultService = "steam", EncryptedPassword = "enc-bad" });
        ...
        results.First(result => result.Id == 1).Password.ShouldBe("password");
        results.First(result => result.Id == 2).Password.ShouldBeEmpty();
    }
}
