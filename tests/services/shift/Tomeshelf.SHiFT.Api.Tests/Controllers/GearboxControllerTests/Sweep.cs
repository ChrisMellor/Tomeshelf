...
public class Sweep
{
    ...
    public async Task ClampsHoursAndMapsResponse()
    {
        ...
        var resultPayload = new ShiftKeySweepResult(
            since,
            scanned,
            new ShiftKeySweepSummary(1, 2, 1, 1),
            new List<ShiftKeySweepItem> { new ShiftKeySweepItem("ABCDE-FGHIJ-KLMNO-PQRST-UVWXY", new[] { "x" }, itemResults) });
        ...
        payload.Items[0].Code.ShouldBe("ABCDE-FGHIJ-KLMNO-PQRST-UVWXY");
        payload.Items[0].Summary.Total.ShouldBe(2);
        payload.Items[0].Summary.Succeeded.ShouldBe(1);
        payload.Items[0].Summary.Failed.ShouldBe(1);
    }
    ...
    public async Task UsesDefaultLookback_WhenHoursNull()
    {
        ...
        var resultPayload = new ShiftKeySweepResult(
            ...
            DateTimeOffset.UtcNow,
            new ShiftKeySweepSummary(0, 0, 0, 0),
            new List<ShiftKeySweepItem>());
