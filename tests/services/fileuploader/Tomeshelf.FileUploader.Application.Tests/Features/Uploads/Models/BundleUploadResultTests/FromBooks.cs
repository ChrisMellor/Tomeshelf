...
public class FromBooks
{
    ...
    public void WithNoBooks_ReturnsZeroCounts()
    {
        ...
        // Assert
        result.UploadedAtUtc.Should().Be(uploadedAt);
        result.BundlesProcessed.Should().Be(0);
        result.BooksProcessed.Should().Be(0);
    ...
    public void DistinctBundles_AreCaseInsensitive()
    {
        ...
        {
            new(bundleName, faker.Commerce.ProductName(), 2, 1),
            new(bundleName.ToLowerInvariant(), faker.Commerce.ProductName(), 1, 0),
            new(faker.Commerce.ProductName(), faker.Commerce.ProductName(), 3, 2)
        };
        ...
        // Assert
        result.BundlesProcessed.Should().Be(2);
        result.BooksProcessed.Should().Be(3);
        result.FilesUploaded.Should().Be(6);
        result.FilesSkipped.Should().Be(3);
    }
}
