...
public class Handle
{
    ...
    public async Task ValidQuery_ReturnsPaissaWorldSummary()
    {
        ...
        A.CallTo(() => settings.WorldId).Returns(worldId);
        ...
        result.WorldName.Should().Be(worldName);
        result.RetrievedAtUtc.Should().Be(now);
        result.Districts.Should().HaveCount(1);
        A.CallTo(() => client.GetWorldAsync(worldId, A<CancellationToken>._))
    ...
    public async Task OrdersDistrictsAndGroupsBySize()
    {
        ...
        A.CallTo(() => settings.WorldId).Returns(7);
        A.CallTo(() => clock.UtcNow).Returns(now);
        ...
        result.Districts[0].Name.Should().Be("Alpha");
        result.Districts[1].Name.Should().Be("beta");

        var sizeGroups = result.Districts[0].SizeGroups;
        sizeGroups.Should().HaveCount(3);
        sizeGroups[0].Size.Should().Be("Large");
        sizeGroups[1].Size.Should().Be("Medium");
        sizeGroups[2].Size.Should().Be("Small");

        var mediumGroup = sizeGroups[1];
        mediumGroup.Plots.Should().HaveCount(1);
        mediumGroup.Plots[0].Ward.Should().Be(1);
        mediumGroup.Plots[0].Plot.Should().Be(1);

        var betaSmallPlots = result.Districts[1].SizeGroups.First(group => group.Size == "Small").Plots;
        betaSmallPlots.Should().HaveCount(1);
        betaSmallPlots[0].Ward.Should().Be(1);
        betaSmallPlots[0].Plot.Should().Be(2);
    }
    ...
    public async Task FiltersUnknownSizes_WhenRequireKnownSize()
    {
        ...
        var now = DateTimeOffset.UtcNow;
        A.CallTo(() => settings.WorldId).Returns(3);
        A.CallTo(() => clock.UtcNow).Returns(now);
        ...
        // Assert
        result.Districts.Should().HaveCount(1);
        result.Districts[0].Name.Should().Be("Known");
    }
}
