...
public class OnModelCreating
{
    ...
    public void BundleEntity_IsConfiguredWithExpectedMetadata()
    {
        ...
        shortDescriptionProperty!.GetMaxLength().ShouldBe(1024);

        entityType.FindProperty(nameof(Bundle.FirstSeenUtc))?.GetColumnType().ShouldBe("datetimeoffset(0)");
        entityType.FindProperty(nameof(Bundle.LastSeenUtc))?.GetColumnType().ShouldBe("datetimeoffset(0)");
        entityType.FindProperty(nameof(Bundle.LastUpdatedUtc))?.GetColumnType().ShouldBe("datetimeoffset(0)");
        entityType.FindProperty(nameof(Bundle.StartsAt))?.GetColumnType().ShouldBe("datetimeoffset(0)");
        entityType.FindProperty(nameof(Bundle.EndsAt))?.GetColumnType().ShouldBe("datetimeoffset(0)");
    }
}
