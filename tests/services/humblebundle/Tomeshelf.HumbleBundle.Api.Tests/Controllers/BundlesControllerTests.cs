...
public class BundlesControllerTests
{
    ...
    public async Task GetBundles_ReturnsOk_WithMappedResponses()
    {
        ...
        var response = ok.Value.Should().BeAssignableTo<IReadOnlyList<BundlesController.BundleResponse>>().Subject.Single();

        response.MachineName.Should().Be(dto.MachineName);
        response.Category.Should().Be(dto.Category);
        response.Stamp.Should().Be(dto.Stamp);
        response.Title.Should().Be(dto.Title);
        response.ShortName.Should().Be(dto.ShortName);
        response.Url.Should().Be(dto.Url);
        response.TileImageUrl.Should().Be(dto.TileImageUrl);
        response.TileLogoUrl.Should().Be(dto.TileLogoUrl);
        response.HeroImageUrl.Should().Be(dto.HeroImageUrl);
        response.ShortDescription.Should().Be(dto.ShortDescription);
        response.StartsAt.Should().Be(dto.StartsAt);
        response.EndsAt.Should().Be(dto.EndsAt);
        response.FirstSeenUtc.Should().Be(dto.FirstSeenUtc);
        response.LastSeenUtc.Should().Be(dto.LastSeenUtc);
        response.LastUpdatedUtc.Should().Be(dto.LastUpdatedUtc);
        response.GeneratedUtc.Should().Be(dto.GeneratedUtc);
        response.SecondsRemaining.Should().BeGreaterThan(0).And.BeLessThan(301);
    ...
    {
        var now = DateTimeOffset.UtcNow;
        var dto = CreateDto(endsAt: now.AddMinutes(-5));
    ...
    public async Task RefreshBundles_ReturnsOk_WithIngestSummary()
    {
        ...
        var ok = result.Result.Should().BeOfType<OkObjectResult>().Subject;
        var response = ok.Value.Should().BeOfType<BundlesController.RefreshBundlesResponse>().Subject;

        response.Created.Should().Be(1);
        response.Updated.Should().Be(2);
        response.Unchanged.Should().Be(3);
        response.Processed.Should().Be(6);
        response.ObservedAtUtc.Should().Be(observed);

        A.CallTo(() => refreshHandler.Handle(A<RefreshBundlesCommand>._, A<CancellationToken>._))
            .MustHaveHappenedOnceExactly();
    }

    private static BundlesController CreateController(
        IQueryHandler<GetBundlesQuery, IReadOnlyList<BundleDto>> queryHandler,
        ICommandHandler<RefreshBundlesCommand, BundleIngestResult> refreshHandler)
    {
