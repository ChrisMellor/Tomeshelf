...
public sealed class FitbitDashboardServiceTests
{
    ...
    public async Task GetDashboardAsync_PopulatesBedtimeAndWakeTime_WhenSleepTimesContainFullTimestamps()
    {
        ...
        dashboard!.Sleep.Bedtime.Should().Be("22:15");
        dashboard.Sleep.WakeTime.Should().Be("06:45");
    }
    ...
    public async Task GetDashboardAsync_ForceRefreshBypassesCacheAndRefetches()
    {
        ...
        first?.Activity.Steps.Should().Be(1);

        var second = await service.GetDashboardAsync(date, false, CancellationToken.None);
        fetchCount.Should().Be(1); // cached result, no new fetch
        second?.Activity.Steps.Should().Be(1);

        var third = await service.GetDashboardAsync(date, true, CancellationToken.None);
        fetchCount.Should().Be(2); // force refresh triggered new fetch
        third?.Activity.Steps.Should().Be(2);
    }
}
