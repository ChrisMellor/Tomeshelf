...
public sealed class PaissaApi : IPaissaApi
{
    ...
    public PaissaApi(HttpClient http, ILogger<PaissaApi> logger)
    {
        _http = http;
        _logger = logger;
    }
    ...
    public async Task<PaissaWorldModel> GetWorldAsync(CancellationToken cancellationToken)
    {
        ...
        var world = await JsonSerializer.DeserializeAsync<PaissaWorldModel>(stream, SerializerOptions, cancellationToken) ??
                    throw new InvalidOperationException("Empty Paissa payload");
