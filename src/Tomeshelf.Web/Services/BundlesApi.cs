...
public sealed class BundlesApi(HttpClient http, ILogger<BundlesApi> logger) : IBundlesApi
{
    ...
    /// <inheritdoc />
    public async Task<IReadOnlyList<BundleModel>> GetBundlesAsync(bool includeExpired,
        CancellationToken cancellationToken)
    {
        ...
        logger.LogInformation("HTTP GET {Url} -> {Status} in {Duration}ms", url, (int)response.StatusCode,
        ...
        var bundles =
            await JsonSerializer.DeserializeAsync<List<BundleModel>>(stream, SerializerOptions, cancellationToken)
