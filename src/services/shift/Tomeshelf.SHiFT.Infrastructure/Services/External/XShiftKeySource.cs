...
public sealed class XShiftKeySource : IShiftKeySource
{
    ...
    public async Task<IReadOnlyList<ShiftKeyCandidate>> GetKeysAsync(DateTimeOffset sinceUtc, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var username in settings.Usernames.Select(u => u?.Trim()).Where(u => !string.IsNullOrWhiteSpace(u)))
        {
    ...
    private async Task<IReadOnlyList<XV2Tweet>> GetUserTweetsV2Async(
        HttpClient client,
        string apiBase,
        string bearerToken,
        ShiftKeyScannerOptions.XSourceOptions settings,
        string userId,
