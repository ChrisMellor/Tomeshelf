...
public sealed record BundleUploadResult
{
    ...
    public void Deconstruct(out DateTimeOffset uploadedAtUtc, out int bundlesProcessed, out int booksProcessed, out int filesUploaded, out int filesSkipped, out IReadOnlyList<BookUploadResult> books)
    {
        uploadedAtUtc = UploadedAtUtc;
        bundlesProcessed = BundlesProcessed;
        booksProcessed = BooksProcessed;
