...
namespace Tomeshelf.Infrastructure.Domains.Fitness.Services;
...
public sealed class FitbitDashboardService
{
    ...
    private static FitbitWeightSummaryDto BuildWeightSummary(WeightResponse response)
    {
        ...
        var data = response.Entries.Where(e => e.Weight != 0)
                           .Select(entry =>
                            {
        ...
        {
            change = startingWeight - currentWeight;
        }
        ...
        if (currentWeight != 0)
        {
            if ((leanMass == 0) && (bodyFat != 0))
            {
                leanMass = Math.Round(currentWeight * (1 - (bodyFat / 100d)), 2);
            }

            if ((bodyFat == 0) && (leanMass != 0) && (currentWeight > 0))
            {
                bodyFat = Math.Round((1 - (leanMass / currentWeight)) * 100d, 2);
            }
        }
    ...
    private static FitbitSleepSummaryDto BuildSleepSummary(SleepResponse response)
    {
        ...
        var entries = response.Entries.Where(e => (e.MinutesAsleep > 0) || (e.MinutesAwake > 0))
                              .ToList();
        ...
        var totalSleepMinutes = entries.Sum(e => e.MinutesAsleep);
        var totalAwakeMinutes = entries.Sum(e => e.MinutesAwake + e.MinutesAfterWakeup + e.MinutesToFallAsleep);
        var efficiencyValues = entries.Select(e => e.Efficiency)
                                      .Where(efficiency => efficiency > 0)
                                      .Select(efficiency => (double)efficiency)
                                      .ToList();
        ...
        var efficiency = efficiencyValues.Count > 0
                ? efficiencyValues.Average()
                : 0;

        static double ToHours(int minutes)
        {
            return minutes > 0 ? Math.Round(minutes / 60d, 2) : minutes == 0 ? 0d : 0;
        }

        int SumLevelMinutes(Func<SleepLevelSummary, SleepLevelSummaryItem> selector)
        {
            var total = 0;
            var hasData = false;
    ...
    private static FitbitActivitySummaryDto BuildActivitySummary(ActivitiesResponse response)
    {
        ...
        {
            return new FitbitActivitySummaryDto(0, 0, 0);
        }

        var distance = response.Summary.Distances.FirstOrDefault(d => string.Equals(d.Activity, "total", StringComparison.OrdinalIgnoreCase))!.Distance;
