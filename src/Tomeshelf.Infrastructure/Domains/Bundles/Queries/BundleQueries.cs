...
namespace Tomeshelf.Infrastructure.Domains.Bundles.Queries;
...
public sealed class BundleQueries
{
    ...
    public async Task<IReadOnlyList<BundleDto>> GetBundlesAsync(bool includeExpired, CancellationToken cancellationToken = default)
    {
        ...
        if (!includeExpired)
        {
            query = query.Where(b => (b.EndsAt == default) || (b.EndsAt >= now));
        }

        var generatedAt = DateTimeOffset.UtcNow;

        return await query.OrderBy(b => b.EndsAt != default
                                           ? 0
                                           : 1)
                          .ThenBy(b => b.EndsAt)
                          .ThenBy(b => b.Title)
